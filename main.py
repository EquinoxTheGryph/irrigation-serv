...
async def main():
    ...
    serial.port = config.get("serial_port")
    serial.baudrate = config.get("serial_baud")
    try:
        serial.open()
...
# Monitor Serial input, parse it and then publish the values to MQTT TODO: WHILE LOOPS ARE VERY CPU INTENSIVE!!
async def monitor_serial_input():
    while not killer.kill_now:
        if serial.inWaiting() > 0:
            # Read Serial input
            _data = serial.readline().decode("utf-8").rstrip()
...
def publish_data(json_obj):
    # Get json data and publish it - If a certain key wasn't recieved, it will not publish any data with that key (avoids sudden null payloads)
...
# Perform this function when a MQTT message has been recieved
def on_message(client, userdata, message):
    try:
        ...
        serial.write(str(str(output_json) + "\n").encode('utf-8'))
        serial.flush()
